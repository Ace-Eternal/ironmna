...
const Dnd = props => {
    const { x = 0, y = 0, z = 'auto', w = 200, h = 28, minW = 28, minH = 28, handlers = ['n', 'e', 's', 'w', 'ne', 'nw', 'se', 'sw'], active = false, canDeactive = true, draggable = true, axis = 'both', grid = [1, 1], parent = false, children, onActivated, onDeactivated, onResizing, onResizeStop, onDraging, onDragStop } = props;
    ...
    const handleActivated = (e) => {
        ...
        if (dndRef.current?.contains(target)) {
            e.stopPropagation();
            e.preventDefault();
            reviewDimensions();
            if (!dndData.enabled) {
    ...
    const handleResizeStart = (e, handler) => {
        e.stopPropagation();
        e.preventDefault();
        setDndData(draft => {
            draft.resizing = true;
    ...
    return (_jsxs("div", { ref: dndRef, className: classNames(styles['dnd-wrapper'], { [styles['draggable']]: draggable, [styles['active']]: active }), style: { ...getDndStyle() }, onMouseDown: e => handleActivated(e), children: [handlers.map(item => (_jsx("div", { className: classNames(styles['dnd-handler'], styles[`handler-${item}`]), style: { display: dndData.enabled ? 'block' : 'none' }, onMouseDown: e => handleResizeStart(e, item) }, item))), children] }));
};
export default Dnd;
